...
if (length(args) != 4) {
  stop("Four command line arguments are required. Please use full paths for now. (1) Name of InOut file for data, (2) Name of outlier file containing unique sources, (3) Sorting type, which can take on three possible values -- 'ip', 'cluster', or 'threat', and (4) File type, which can take on three possible values -- 'pdf', 'png', or 'jpg'.")
}
...
# FINISHED
# Increase size of points. Changed from pch='.' to pch=15 (which is a square)
...
MakeCircs <- function(dataFile, outlierFile, fileType, sortKey="ip") {
  ...
  # Creating image file which plots will be saved to
  if (missing(fileType) || all(fileType=="png")){
    file <- paste0(file,".png")
    png(file, width = 300*rows, height = 300*cols)
  } else if (all(fileType=="jpeg") || all(fileType=="jpg")){
    file <- paste0(file,".jpeg")
    jpeg(file, width = 300*rows, height = 300*cols)
  } else if (all(fileType=="pdf")){
    file <- paste0(file,".pdf")
    pdf(file, width = 3*rows, height = 3*cols)
  }
  par(mfrow = c(rows,cols), oma = c(1,0,2,0))
  ...
  for (i in 1:nrow(uniqueSources)) {
    ...
    title(main = source)
    if (i == 1) {
      title(main = plotsTitle, outer = TRUE)
    }
    ...
    # Draw the connecting lines between sections (not dependent on number of sectors)
    for (j in 1:nrow(connections)) {
      currentFactor <- (connectionMapping %>% filter(DIP==connections$DIP[j]))$sector[1] # sector number of DIP
      if (connections$RPacketCount[j] == 0) {
  ...
  # If the file type was not empty, save the file
  if (fileType != "") {
...
tic()
MakeCircs(dataFile, outlierFile, fileType, sortType)
toc()
