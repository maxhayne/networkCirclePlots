...
MakeCircs <- function(outlierFile, fileType=".png", sortKey="ip") {
  ...
  plot.list <- foreach (i = 1:nrow(uniqueSources)) %dopar% {
    ...
    circos.clear()
    dev.off() # save the plot to the file
    img <- as.raster(readPNG(tempName))
    # unlink(tempName) # cleanup of tmp directory may be done automagically
    plot <- rasterGrob(img, interpolate = FALSE) # this will be what is combined in plot.list
  }
