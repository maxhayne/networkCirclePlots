...
registerDoParallel(cores=coreCount-10)
...
MakeCircs <- function(outlierFile, fileType=".png", sortKey="ip") {
  ...
  plot.list <- foreach (i = 1:nrow(uniqueSources)) %dopar% {
    ...
    connections <- df %>% filter(SIP == .subset2(uniqueSources,1)[i]) %>% arrange(DIP)
    connectionMapping <- connections %>% distinct(DIP)  %>% mutate(sector = row_number()+1) # row_number+1 = section in circle plot
    df <- df %>% filter(SIP != .subset2(uniqueSources,1)[i]) # Filtering out all the entries we just selected, for speed on subsequent ops
    
    destinationCount <- connections %>% distinct(DIP) %>% tally()
    factors <- c(1:(.subset2(destinationCount,1)[1] + 1)) # Weird syntax for finding the total number of slices in circle plot
    source <- as.character(.subset2(uniqueSources,1)[i]) # Grabbing the source IP
    numFactors <- as.integer(.subset2(destinationCount,1)[1] + 1)
    ...
    if (numFactors > 200) {
      ...
      if (numFactors >= 2500) { # darkest grey for any number of destinations greater than 2500
        ...
        greyNumber <- 80 - as.integer(((numFactors-100)/2400)*80) # 2500 will use grey0, 100 will use grey80
    ...
    if (numFactors <= 200) {
      for (j in 1:nrow(connections)) {
        currentFactor <- (connectionMapping %>% filter(DIP==.subset2(connections,3)[j]))$sector[1] # sector number of DIP
        circos.points(x = .subset2(connections,1)[j], y = .subset2(connections,6)[j], sector.index = 1, col = "#7B3294", pch = 20)
        if (.subset2(connections,8)[j] != 0) {
          circos.points(x = .subset2(connections,1)[j], y = .subset2(connections,8)[j], sector.index = currentFactor, col = "#7B3294", pch = 20)
          circos.link(currentFactor, .subset2(connections,1)[j], 1, .subset2(connections,1)[j], col = "#5AB4AC")
        } else {
          circos.link(currentFactor, .subset2(connections,1)[j], 1, .subset2(connections,1)[j], col = "#D8B365")
        }
      }
    } else {
      # The 'connections' data frame is sorted by DIP, so we can keep a running count of how many links we've drawn for each DIP.
      # The logic here is that since we're using lines to represent destinations instead of sectors, all links drawn to the same 
      # destination will be drawn on top of one another. So, if there exists a teal line from the SIP to the DIP, we want to show it.
      # If there isn't a teal line, we want to draw an amber link, but a maximum of one, as there's no point in drawing over what is
      # already there. This should save time for datasets with lots of destinations, and lots of points per destination.
      plottedTeal <- FALSE
      plottedAmber <- FALSE
      thisFactor <- -1
      for (j in 1:nrow(connections)) {
        currentFactor <- (connectionMapping %>% filter(DIP==.subset2(connections,3)[j]))$sector[1] # sector number of DIP
        if (currentFactor == thisFactor) {
          if (.subset2(connections,8)[j] != 0) {
            if (!plottedTeal) { # draw teal if we haven't already
              circos.link(2, currentFactor, 1, 0, col = "#5AB4AC")
              plottedTeal <- TRUE
            }
          } else {
            if (!plottedAmber) { # draw amber if we haven't already
              circos.link(2, currentFactor, 1, 0, col = "#D8B365")
              plottedAmber <- TRUE
            }
          }
        } else { # this is a new destination
          thisFactor <- currentFactor
          if (.subset2(connections,8)[j] != 0) { # draw teal, set plottedTeal to true, plottedAmber to false
            circos.link(2, currentFactor, 1, 0, col = "#5AB4AC")
            plottedTeal <- TRUE
            plottedAmber <- FALSE
          } else { # draw amber, set plottedAmber to true, plottedTeal to false
            circos.link(2, currentFactor, 1, 0, col = "#D8B365")
            plottedAmber <- TRUE
            plottedTeal <- FALSE
          }
  ...
  ggsave(fileCombined, width=8.5, height=11,
         arrangeGrob(grobs=plot.list, nrow=rows, ncol=cols, top=textGrob(as.character(plotsTitle), gp=gpar(fontsize=10))))
}
