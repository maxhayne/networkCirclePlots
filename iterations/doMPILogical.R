...
cl <- startMPIcluster(count=coreCount-4,logdir="")
registerDoMPI(cl)
...
MakeCircs <- function(outlierFile, fileType=".png", sortKey="ip") {
  ...
  plot.list <- foreach (i = 1:nrow(uniqueSources), .packages=c("dplyr","circlize","stringr","png","grid"), .options.mpi=list(chunkSize=1)) %dopar% {
