...
MakeCircs <- function(outlierFile, fileType=".png", sortKey="ip") {
  ...
  plotsTitle <- anytime(as.integer(strsplit(file, split = "_")[[1]][1])*60, asUTC=TRUE)
  ...
  plot.list <- foreach (i = 1:nrow(uniqueSources)) %dopar% {
    ...
    #tic()
    #dataTableConnections <- as.data.table(connections)
    #setkey(dataTableConnections,DIP)
    ...
    if (numFactors <= 500) {
      for (j in 1:nrow(connections)) {
        ...
        #tic()
        #currentFactor <- dataTableMapping[.(dataTableConnections[j,DIP]), nomatch = 0L]$sector[1]
        #toc()
        #show(currentFactor)
    ...
    # Will need to modify this to more than csu's network, figure out if the naming scheme of the outlier and InOut files
    # will hold, which now includes the network prefix
