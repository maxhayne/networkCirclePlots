...
MakeCircs <- function(outlierFile, fileType=".png", sortKey="ip") {
  ...
  # Divvy up the plots based on the number of destinations and the coreCount
  tic()
  groupedCounts <- df %>% distinct(SIP,DIP) %>% group_by(SIP) %>% count() %>% arrange(desc(n))
  groupedCounts[] <- lapply(groupedCounts, as.character)
  print(groupedCounts)
  if (coreCount <= sourceCount) {
    listLength <- coreCount
  } else {
    listLength <- sourceCount
  }
  newdf <- data.frame(SIP=character(0),mapping=integer(0))
  stop <- FALSE
  index <- 1
  while (stop == FALSE) {
    for (i in index:(index+listLength)) {
      if (index > sourceCount) {
        stop = TRUE
        break
      }
      newdf <- newdf %>% add_row(SIP=groupedCounts$SIP[i], mapping=i)
      index <- index+1
    }
    if (stop) {
      break
    }
    if (index + listLength > sourceCount) {
      reverse_index <- sourceCount
    } else {
      reverse_index <- index + listLength
    }
    for (i in reverse_index:index) {
      if (index > sourceCount) {
        stop = TRUE
        break
      }
  ...
  # switched from to_compute=chunked_newdf
  plot.list <- foreach (i = 1:nrow(newdf)) %dopar% {
    ...
    connections <- df %>% filter(SIP == newdf$SIP[i]) %>% arrange(DIP)
    connectionMapping <- connections %>% distinct(DIP)  %>% mutate(sector = row_number()+1) # row_number+1 = section in circle plot
    df <- df %>% filter(SIP != newdf$SIP[i]) # Filtering out all the entries we just selected, for speed on subsequent ops
    ...
    source <- as.character(newdf$SIP[i]) # Grabbing the source IP
    ...
    if (numFactors <= 200) {
      ...
      for (j in 1:nrow(connections)) {
        ...
        if (currentFactor == thisFactor) {
          ...
          if (connections$RPacketCount[j] != 0) { # draw teal, set plottedTeal to true, plottedAmber to false
            circos.link(2, currentFactor, 1, 0, col = "#5AB4AC")
            plottedTeal <- TRUE
            plottedAmber <- FALSE
